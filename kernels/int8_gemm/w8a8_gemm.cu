...
torch::Tensor matmul_w8a8(const torch::Tensor &A, const torch::Tensor &B, const torch::Tensor &alphaCol, const torch::Tensor &alphaRow) {
    torch::checkAllSameGPU("W8A8Matmul", {{A, "A", 0}, {B, "B", 1}});
    int32_t M = A.size(0);
    int32_t N = B.size(0);
